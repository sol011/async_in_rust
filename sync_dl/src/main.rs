...
fn sync_download(res: impl Iterator<Item = reqwest::blocking::Response>, cwd: PathBuf) {
    for r in res {
...
fn main() {
    ...
    let maybe = res.into_iter()
        .filter_map(|e| e.ok());
    let success = maybe.filter(|e| e.status() == reqwest::StatusCode::OK);
    sync_download(success, cwd);
